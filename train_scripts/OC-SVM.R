...
#X<-dataset[, c(1,2,3,4,5,6,7)]
#Y<-test[, c(1,2,3,4,5,6,7)]
...
#experiment3
#model <- svm(x, y,type='one-classification', kernel="radial", nu =0.4, gamma= (1 / ncol(x)))  #train an one-classification model

#experiment4
#model <- svm(x, y,type='one-classification', kernel="radial", nu =0.2, gamma= (1 / ncol(x)))  #train an one-classification model


#experiment5
#model <- svm(x, y,type='one-classification', kernel="radial", nu =0.7, gamma= (1 / ncol(x)))  #train an one-classification model
...
name <- paste('./ZUM/svm_oneclass/svm_roc_plot_exp5',  '.jpg', sep = "", collapse = NULL)
jpeg(name)
plot(roc)
dev.off()
...
name <- paste('./ZUM/svm_oneclass/svm_pr_plot_exp5', '.jpg', sep = "", collapse = NULL)
jpeg(name)
plot(pr)
dev.off()
